...
use ezui::widget::*;
use ezui::mouse::*;
use ezui::glium::backend::glutin_backend::GlutinFacade;
...
impl MyUi {
    pub fn new (display: GlutinFacade) -> Self{
        ...
        let texture_raw_attack_knob_base = SimpleTexture::from(&KNOB_BASE_WHITE_RAW, ImageFormat::PNG, &display).unwrap();
        let texture_raw_attack_knob_light =SimpleTexture::from(&KNOB_LIGHT_RAW, ImageFormat::PNG, &display).unwrap();
        ...
        let texture_attack_knob_base = UiTextureBuilder::default().position(KNOB_ATTACK_POSITION).size(KNOB_ATTACK_SIZE).rotation(KNOB_ATTACK_ROTATION).texture(texture_raw_attack_knob_base).build().unwrap();
        let texture_attack_knob_light= UiTextureBuilder::default().position(KNOB_ATTACK_POSITION).size(KNOB_ATTACK_SIZE).rotation(KNOB_ATTACK_ROTATION).texture(texture_raw_attack_knob_light).build().unwrap();
        let button_attack_knob = UiButtonBuilder::default().position(KNOB_ATTACK_POSITION).size(KNOB_ATTACK_SIZE).build().unwrap();
        ...
        Self {
            ...
            texture_attack_knob_base:            texture_attack_knob_base,
            texture_attack_knob_light:            texture_attack_knob_light,
            button_attack_knob:            button_attack_knob,
        }
    }
